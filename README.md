# César Cuenca Pérez 

Programador Java en formación con experiencia en desarrollo de software, bases de datos, sistemas y desarrollo web.

Motivado por seguir creciendo profesionalmente y aplicar mis conocimientos en entornos reales de desarrollo.

---

##  Sobre mí

Desarrollador en formación con interés en:

- Desarrollo de aplicaciones Java
- Bases de datos relacionales
- Desarrollo web frontend
- Administración básica de sistemas
- Proyectos full-stack educativos

---

##  Habilidades técnicas

- Programación orientada a objetos (Java)
- Desarrollo de interfaces gráficas (Swing)
- Diseño y gestión de bases de datos (MySQL)
- Desarrollo web (HTML, CSS, JavaScript)
- Uso de control de versiones (Git y GitHub)
- Resolución de problemas en entornos reales

---

##  Herramientas y entornos de desarrollo

![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-000000.svg?style=for-the-badge&logo=intellij-idea&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![MySQL Workbench](https://img.shields.io/badge/MySQL_Workbench-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white)

- IDEs: IntelliJ IDEA, Visual Studio Code  
- Bases de datos: MySQL Workbench  

---

##  Lenguajes y tecnologías

![Java](https://img.shields.io/badge/Java-ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white)
![JSON](https://img.shields.io/badge/JSON-000000.svg?style=for-the-badge&logo=json&logoColor=white)
![Markdown](https://img.shields.io/badge/Markdown-000000.svg?style=for-the-badge&logo=markdown&logoColor=white)

---

##  Proyectos relevantes

###  El Pensador
Juego tipo ahorcado por categorías.

**Características:**
- Selección de categoría (frutas / animales)
- Palabra aleatoria
- Máximo 6 errores
- Control de letras repetidas

🔗 https://github.com/cesarCuenca1993/El-pensador

---

###  Pet Virtual
Simulador de mascota virtual con estados dinámicos.

**Características:**
- Atributos de la mascota
- Acciones: comer, dormir, jugar, bañar
- Sistema de estados y evolución
- Condiciones de final de partida

🔗 https://github.com/cesarCuenca1993/Pet-virtual

---

###  Football Manager
Simulador de gestión deportiva.

**Características:**
- Gestión de equipos
- Alta y baja de jugadores y entrenadores
- Creación de ligas
- Administración de estructuras deportivas

🔗 https://github.com/cesarCuenca1993/Footbal-M

---

###  Quiz Protectora Animal
Juego de preguntas tipo trivia.

**Características:**
- Elección de temática (perro o gato)
- Preguntas aleatorias
- Número de preguntas configurable
- Sistema de vidas (3 errores máximo)

🔗 https://github.com/cesarCuenca1993/mi-quiz

---

###  Web Protectora Animal
Página web informativa sobre adopción de animales.

**Características:**
- Historias de adopción
- Fichas de animales
- Diseño informativo y educativo
- Concienciación sobre adopción responsable

🔗 https://github.com/cesarCuenca1993/web_estatica
